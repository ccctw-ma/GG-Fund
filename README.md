...
- Next.js App Router 页面：根路径 `/` 会直接跳转到 `/app` 工作台，`/login` 提供独立邮箱验证码登录页，`/architecture` 以固定深色控制台风格展示项目技术架构，并用系统架构图、行情请求时序图和接口流程图解释浏览器、Route Handler、Cloudflare Worker、市场数据源、AI/OCR 和 D1 存储之间的关系；`/settings` 目前提供基础说明入口，`/app/portfolio` 当前作为组合落地页；本地持仓与自选能力继续在工作台内使用。
- 顶部导航工作台：`/app` 内通过固定顶部导航在“行情、账户”之间切换；`/architecture` 页面内容保留但暂不在顶部导航暴露入口；右上角展示紧凑账户状态卡片；未登录时点击右上角“登录”进入 `/login`。原“总览”落地页和功能卡片区已移除，进入 `/app` 默认展示行情页。
...
- 全球指数行情：行情页首屏以横向可点击卡片展示 A 股、港股、美股、日经、韩国和欧洲主要指数，默认选中上证指数，并在卡片下方用全宽走势图展示当前指数历史走势；实时点位优先使用东方财富 `push2` 的 `ulist.np/get`，全球指数使用 `100.{code}` 形式的 secid，例如 `100.DJIA`、`100.SPX`、`100.NDX`、`100.N225`、`100.KS11`、`100.FTSE`、`100.GDAXI`、`100.FCHI`、`100.HSI`；当东方财富在 Worker 出口不可用时，接口会合并腾讯 A 股指数和新浪全球指数行情作为兜底，并用历史日线补齐科创 50、北证 50、纳斯达克 100 等必备指数卡片。全球指数历史走势优先尝试东方财富 `push2his`，失败后使用 Naver Chart API 的公开日线接口（如 `.DJI`、`.INX`、`.IXIC`、`.NDX`、`.N225`、`KOSPI`、`.HSI`、`.FTSE`、`.GDAXI`、`.FCHI`）；科创 50 和北证 50 历史走势额外使用搜狐历史行情接口兜底，避免东财或腾讯历史数据为空。
- 金融资产搜索：作为工作台第二板块放在指数行情下方，按代码或名称查询公开基金数据和 A 股实时行情，搜索结果以横向可切换卡片展示，接口失败时自动回退内置示例行情。
- 基金详情：优先展示天天基金 `fundgz` 盘中估算净值、估算涨跌和估算时间，同时保留上一交易日官方净值；当估算接口或移动端净值接口不可用时，会继续尝试东方财富 PC `pingzhongdata` 净值页和基金搜索精确命中，按估算时间/净值日期选择最新可用行情，避免单一上游抖动导致 404 或旧数据；详情动作区和持仓明细都提供「智能分析」入口，点击后会从右侧打开全高抽屉，使用更宽松的版式展示完整走势研判、全部情景、关注点、分析依据和来源链接，不再使用可拖拽缩放的小浮层。前端默认调用 `/api/ai/analyze-fund/stream` 读取流式分析草稿，服务端会从 Cloudflare runtime context 读取 `DEEPSEEK_API_KEY`，联网抓取基金行情、历史走势、主要指数和东方财富公开网页材料，再交给 Deepseek 流式生成涨跌驱动、后续走势因素、关注点和来源链接；初始报告下方提供「继续追问」对话框，会通过 `POST /api/ai/analyze-fund/follow-up` 带上当前报告摘要继续向 DeepSeek 提问，未配置 key 时同样返回基于现有行情和报告上下文的本地回退回答；缺少 key 时会明确降级到本地确定性报告，不再伪装成 AI 已就绪。
- 基金分析走势图：基金详情页使用与主界面一致的深色玻璃 ECharts 卡片，默认只展示点位折线和“五日、日K、周K、半年、月K、更多”的时间切换，K 线保留在默认指标区作为独立开关，用户需要时可手动打开，不再默认遮挡点位趋势；MA5、MA10、MA20 均线不再展示，进一步保持图表简洁。默认/可选指标工具栏继续保留，区间收益、最大回撤、年化收益、夏普、波动率、基准收益和超额收益默认隐藏、按需叠加，避免额外线条干扰首屏读图。点位线参考公开金融图表的渐变面积和高亮端点做法，使用高对比浅色主题线、柔和光晕、渐隐面积填充和最新值尾标，K 线打开后沿用主题中的红涨绿跌实色蜡烛，坐标轴、网格线和 Tooltip 统一为深色主题。少量数据点使用固定窄宽度，并用前一收盘点生成合成蜡烛的开盘价，影线只做轻量补足，减少“针刺感”和失真的 K 线实体。Tooltip、坐标轴和十字指针统一格式化数值，避免展示浮点长尾。指数、基金、个股历史走势都支持按需叠加 K 线；当上游提供开高低收时优先使用真实 OHLC，A 股个股的腾讯日 K 兜底也会保留真实开盘、收盘、最高和最低价；当只提供净值/收盘点位时，前端围绕当前收盘价生成窄幅净值蜡烛。历史走势请求带版本参数以刷新旧缓存中的非 OHLC 数据。
