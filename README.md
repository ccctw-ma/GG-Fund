...
- 基金分析走势图：基金详情页使用暗色数据雷达风格的 ECharts 图表，默认展示 K 线、收盘价/净值、区间收益和最大回撤；年化收益、夏普、波动率、相对沪深 300 基准收益和超额收益作为可选指标点击展开，并保留时间范围切换。指数、基金、个股历史走势都会叠加 K 线；当上游只提供净值/收盘点位时，前端按相邻点推导开收高低作为类 K 线展示。
