...
- 基金详情：优先展示天天基金盘中估算净值、估算涨跌和估算时间，同时保留上一交易日官方净值；详情动作区和持仓明细都提供「智能分析」入口，点击后会打开可拖拽、可缩放的右侧浮层，调用 `/api/ai/analyze-fund`。服务端会读取基金行情、历史走势、主要指数和东方财富公开网页材料，再交给 Deepseek 生成精简版的涨跌驱动、后续走势因素、关注点和来源链接。
