...
- 基金分析走势图：基金详情页使用与主界面一致的深色玻璃 ECharts 卡片，默认只展示点位折线和“五日、日K、周K、半年、月K、更多”的时间切换，K 线改为图例区的独立开关，用户需要时可手动打开，不再默认遮挡点位趋势；MA5、MA10、MA20 均线也不再展示，进一步保持图表简洁。不再展示默认/可选指标工具栏，避免区间收益、最大回撤、年化收益、夏普、波动率、基准收益和超额收益等额外线条干扰读图。点位线使用高对比浅色主题线，K 线打开后沿用主题中的红涨绿跌实色蜡烛，坐标轴、网格线和 Tooltip 统一为深色主题。少量数据点使用固定窄宽度，并用前一收盘点生成合成蜡烛的开盘价，影线只做轻量补足，减少“针刺感”和失真的 K 线实体。Tooltip、坐标轴和十字指针统一格式化数值，避免展示浮点长尾。指数、基金、个股历史走势都支持按需叠加 K 线；当上游提供开高低收时优先使用真实 OHLC，A 股个股的腾讯日 K 兜底也会保留真实开盘、收盘、最高和最低价；当只提供净值/收盘点位时，前端围绕当前收盘价生成窄幅净值蜡烛。历史走势请求带版本参数以刷新旧缓存中的非 OHLC 数据。
