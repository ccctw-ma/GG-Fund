...
- 基金详情：优先展示天天基金盘中估算净值、估算涨跌和估算时间，同时保留上一交易日官方净值；详情动作区提供「智能分析」入口，点击后在右侧 fixed 面板调用 `/api/ai/analyze-fund`，由服务端读取基金行情、历史走势、主要指数和东方财富公开网页材料，再交给 Deepseek 生成涨跌原因、未来走势因素、情景推演、关注点和来源摘要。
