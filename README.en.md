...
- Fund analysis trend chart: fund details use a dark data-radar ECharts visualization whose default indicators are point value and candlesticks; the point-value line stays thin, subdued, and below the candles so it does not dominate the chart. Candlesticks use a TradingView-like solid body and same-color wick treatment, keep a fixed narrow width for sparse ranges, and use shorter synthetic candle bodies and wicks to reduce visual noise. Range return, max drawdown, annualized return, Sharpe ratio, volatility, CSI 300 benchmark return, and excess return are optional chart indicators that can be toggled while keeping time-range switching. Tooltips, axis labels, and crosshair labels share formatted numbers to avoid floating-point tails. Index, fund, and stock history charts all overlay candlesticks; when upstream data provides OHLC it is used directly, Tencent daily kline stock-history fallback preserves real open, close, high, and low values, and when only NAV/close points are available the frontend creates a narrow NAV candle around the current close instead of treating the previous close as today's open. Historical trend requests include a version parameter to refresh stale non-OHLC caches.
