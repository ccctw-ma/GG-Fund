...
- Fund analysis trend chart: fund details use a dark data-radar ECharts visualization with more prominent candlesticks and a thinner close/NAV line enabled by default; range return, max drawdown, CSI 300 benchmark return, and excess return are optional chart indicators that can be toggled while keeping time-range switching. Index, fund, and stock history charts all overlay candlesticks; when upstream data only provides NAV/close points, the frontend derives an approximate OHLC candle from adjacent points.
