...
- Fund analysis trend chart: fund details now use a dark glass ECharts card consistent with the main workspace theme, showing only the point-value line and the compact Chinese-market range labels 五日, 日K, 周K, 半年, 月K, and 更多 by default. Candlesticks are now controlled by an explicit K-line toggle in the legend strip so users can turn them on when needed without obscuring the default point trend; MA5, MA10, and MA20 moving-average lines are no longer shown either, keeping the chart simpler. The chart no longer shows the default/optional indicator toolbar, so range return, max drawdown, annualized return, Sharpe ratio, volatility, benchmark return, and excess return no longer add noisy extra lines to the Signal Matrix. The point line uses a high-contrast light theme color; enabled candlesticks use the theme's red-for-up and green-for-down solid bodies; axes, grid lines, and tooltips share the dark theme. Sparse ranges keep fixed narrow candle widths, synthetic NAV candles derive opens from the previous close, and wicks are only lightly extended to avoid spiky or distorted candle shapes. Tooltips, axis labels, and crosshair labels share formatted numbers to avoid floating-point tails. Index, fund, and stock history charts support optional candlestick overlays; when upstream data provides OHLC it is used directly, Tencent daily kline stock-history fallback preserves real open, close, high, and low values, and when only NAV/close points are available the frontend creates a narrow NAV candle around the current close. Historical trend requests include a version parameter to refresh stale non-OHLC caches.
