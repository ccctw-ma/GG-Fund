...
- Top navigation workspace: `/app` switches between Market and Account through a sticky top nav and also exposes an Architecture entry that jumps to `/architecture`; a compact account status card stays in the top-right corner, and clicking the top-right Login button opens `/login`. The previous Overview landing page and feature-card section have been removed, so `/app` now opens the Market page by default.
