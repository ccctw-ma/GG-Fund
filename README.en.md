...
- Fund details: prefers Tiantian intraday estimated NAV, estimated change, and estimate time while keeping the previous official NAV; both the detail actions and holding rows expose an "AI analysis" entry that opens a draggable, resizable right-side floating panel and calls `/api/ai/analyze-fund`. The server reads the fund quote, historical trend, major indices, and Eastmoney public web pages, then asks Deepseek to generate a compact view of move drivers, forward factors, watch points, and source links.
