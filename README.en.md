...
- Fund analysis trend chart: fund details now use a lighter market-app-style white ECharts card whose default indicators are point value and candlesticks; candlesticks use softer red-for-up and green-for-down solid bodies and overlay low-saturation MA5, MA10, and MA20 moving-average lines, while the range switcher uses compact Chinese-market labels such as 五日, 日K, 周K, 半年, 月K, and 更多. The palette is intentionally balanced so the chart feels close to a quotes app without overly loud colors. The point-value line stays thin, subdued, and below the candles so it does not dominate the chart. Sparse ranges keep fixed narrow candle widths, and synthetic candle bodies and wicks remain shortened to reduce visual noise. Range return, max drawdown, annualized return, Sharpe ratio, volatility, CSI 300 benchmark return, and excess return are optional chart indicators that can be toggled while keeping time-range switching. Tooltips, axis labels, and crosshair labels share formatted numbers to avoid floating-point tails. Index, fund, and stock history charts all overlay candlesticks; when upstream data provides OHLC it is used directly, Tencent daily kline stock-history fallback preserves real open, close, high, and low values, and when only NAV/close points are available the frontend creates a narrow NAV candle around the current close instead of treating the previous close as today's open. Historical trend requests include a version parameter to refresh stale non-OHLC caches.
