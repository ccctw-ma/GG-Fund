...
- Fund analysis trend chart: fund details use a dark data-radar ECharts visualization with more prominent candlesticks and a thinner close/NAV line enabled by default; range return, max drawdown, annualized return, Sharpe ratio, volatility, CSI 300 benchmark return, and excess return are optional chart indicators that can be toggled while keeping time-range switching. Index, fund, and stock history charts all overlay candlesticks; when upstream data provides OHLC it is used directly, and when only NAV/close points are available the frontend creates a narrow NAV candle around the current close instead of treating the previous close as today's open.
