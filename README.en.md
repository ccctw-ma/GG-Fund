...
- Fund analysis trend chart: fund details use a dark data-radar ECharts visualization with more prominent candlesticks and a thinner close/NAV line enabled by default; candlesticks use a TradingView-like solid body and same-color wick treatment and render above the auxiliary close line. Range return, max drawdown, annualized return, Sharpe ratio, volatility, CSI 300 benchmark return, and excess return are optional chart indicators that can be toggled while keeping time-range switching. Tooltips, axis labels, and crosshair labels share formatted numbers to avoid floating-point tails. Index, fund, and stock history charts all overlay candlesticks; when upstream data provides OHLC it is used directly, and when only NAV/close points are available the frontend creates a narrow NAV candle around the current close instead of treating the previous close as today's open.
