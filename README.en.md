...
- Next.js App Router workspace: the root path `/` redirects directly to the `/app` workspace, `/login` provides a standalone email OTP sign-in page, `/architecture` visualizes the project technical architecture as a dedicated fixed-dark console subpage with a system architecture diagram, market-request sequence diagram, and API flow diagram covering the browser, Route Handlers, Cloudflare Worker, market providers, AI/OCR, and D1 storage; `/settings` currently provides a basic information entry, and `/app/portfolio` currently serves as a landing page while local-first holdings and watchlist stay in the main workspace.
- Top navigation workspace: `/app` switches between Market and Account through a sticky top nav; the `/architecture` page content remains available but is no longer exposed from the top navigation for now; a compact account status card stays in the top-right corner, and clicking the top-right Login button opens `/login`. The previous Overview landing page and feature-card section have been removed, so `/app` now opens the Market page by default.
...
- Fund details: prefers Tiantian Fund `fundgz` intraday estimated NAV, estimated change, and estimate time while keeping the previous official NAV; when the estimate endpoint or mobile NAV endpoint is unavailable, it keeps trying Eastmoney PC `pingzhongdata` NAV data and exact fund-search matches, then chooses the freshest available quote by estimate time or NAV date to avoid 404s or stale data from a single upstream. Both the detail actions and holding rows expose an "AI analysis" entry that opens a full-height right-side drawer with a roomier layout for the complete trend readout, all scenarios, watch points, reasoning notes, and source links instead of the previous draggable floating panel. The frontend now calls `/api/ai/analyze-fund/stream` to render a live drafting view, while the server reads `DEEPSEEK_API_KEY` from the Cloudflare runtime context, collects the fund quote, historical trend, major indices, and Eastmoney public web pages, then asks Deepseek to stream concise drivers, forward factors, watch points, and source links. A "continue asking" chat box appears under the initial report and posts follow-up questions with the current report context to `POST /api/ai/analyze-fund/follow-up`; when the key is missing, follow-up answers also fall back to deterministic local context instead of pretending AI is ready.
- Fund analysis trend chart: fund details now use a dark glass ECharts card consistent with the main workspace theme, showing only the point-value line and the compact Chinese-market range labels 五日, 日K, 周K, 半年, 月K, and 更多 by default. Candlesticks remain available as an explicit K-line toggle in the default indicator group so users can turn them on when needed without obscuring the default point trend; MA5, MA10, and MA20 moving-average lines are no longer shown, keeping the chart simpler. The default/optional indicator toolbar remains available: range return, max drawdown, annualized return, Sharpe ratio, volatility, benchmark return, and excess return are hidden by default but can be overlaid on demand instead of adding noisy extra lines to the first view. The point line follows common public finance-chart patterns with a gradient area fill, soft glow, high-contrast light stroke, and latest-value end label; enabled candlesticks use the theme's red-for-up and green-for-down solid bodies; axes, grid lines, and tooltips share the dark theme. Sparse ranges keep fixed narrow candle widths, synthetic NAV candles derive opens from the previous close, and wicks are only lightly extended to avoid spiky or distorted candle shapes. Tooltips, axis labels, and crosshair labels share formatted numbers to avoid floating-point tails. Index, fund, and stock history charts support optional candlestick overlays; when upstream data provides OHLC it is used directly, Tencent daily kline stock-history fallback preserves real open, close, high, and low values, and when only NAV/close points are available the frontend creates a narrow NAV candle around the current close. Historical trend requests include a version parameter to refresh stale non-OHLC caches.
