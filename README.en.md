...
- Fund details: prefers Tiantian intraday estimated NAV, estimated change, and estimate time while keeping the previous official NAV; the detail actions include an "AI analysis" entry that opens a right-side fixed panel and calls `/api/ai/analyze-fund`. The server reads the fund quote, historical trend, major indices, and Eastmoney public web pages, then asks Deepseek to generate move drivers, forward factors, scenarios, watch points, and source notes.
