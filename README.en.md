...
- Fund analysis trend chart: fund details use a dark data-radar ECharts visualization with candlesticks, close/NAV, range return, and max drawdown enabled by default; annualized return, Sharpe ratio, volatility, CSI 300 benchmark return, and excess return are optional indicators that can be toggled for detail while keeping time-range switching. Index, fund, and stock history charts all overlay candlesticks; when upstream data only provides NAV/close points, the frontend derives an approximate OHLC candle from adjacent points.
